...
class ExplainabilityAgent(BaseAgentAsync):
        ...
        if hasattr(scoring_result, 'metrics') and scoring_result.metrics:
            for metric_name, metric_value in scoring_result.metrics.items():
                factor = ExplanationFactor(
                    ...
                    contribution=self._calculate_contribution(metric_name, metric_value, scoring_result.score),
    ...
    async def _generate_factor_analysis(
        ...
        if context.signals:
            signal_factors = await self._analyze_signal_factors(context.signals, scoring_result.score)
            factors.extend(signal_factors)
        
        # Analyze entity-based factors
        if context.entity:
            entity_factors = await self._analyze_entity_factors(context.entity, scoring_result.score)
    ...
    def _identify_root_causes(
        ...
        if context.signals:
            ...
            critical_vulns = [s for s in context.signals if s.signal_type == "VULNERABILITY" and s.severity == "critical"]
            if critical_vulns:
                root_causes.append({
    ...
    def _generate_comparisons(
        ...
        entity_type_avg = self._get_entity_type_average(context.entity.entity_type if context.entity else "unknown")
        if entity_type_avg:
        ...
        historical_avg = self._get_historical_average(context.entity.id if context.entity else "unknown")
        if historical_avg:
    ...
    def _calculate_contribution(
        ...
        weight = self.factor_weights.get(metric_name, 0.1)
        normalized_value = min(1.0, abs(metric_value) / 100.0) if isinstance(metric_value, (int, float)) else 0.5
    ...
    def _get_factor_evidence(self, metric_name: str, context: Context) -> List[str]:
        ...
        if context.signals:
            relevant_signals = [s for s in context.signals if metric_name.lower() in s.signal_type.lower()]
    ...
    def _calculate_explanation_confidence(self, factors: List[ExplanationFactor]) -> ConfidenceLevel:
    ...
    def _generate_next_steps(self, factors: List[ExplanationFactor]) -> List[str]:
        ...
        for factor in sorted_factors[:3]:
            if factor.direction == "negative":
                steps.append(f"Investigate and mitigate {factor.name} (contribution: {factor.contribution:.2f})")
    ...
    def _generate_factor_recommendations(self, factors: List[ExplanationFactor]) -> List[str]:
        ...
        if risk_factors:
            recommendations.append(
                f"Focus on {len(risk_factors)} primary risk drivers"
            )
        ...
        if positive_factors:
            recommendations.append(
                f"Maintain {len(positive_factors)} positive security factors"
            )
    ...
    def _generate_temporal_recommendations(self, temporal_data: Dict[str, Any]) -> List[str]:
        ...
        if "signal_frequency" in temporal_data:
            ...
            if freq > 20:
                recommendations.append("Investigate high signal frequency - may indicate active issues")
    ...
    def _calculate_severity_trend(self, signals: List[Signal]) -> Dict[str, Any]:
        ...
        recent_avg = sum(severity_values.get(s.severity.lower(), 1) for s in signals[-10:]) / min(10, len(signals))
        older_avg = sum(severity_values.get(s.severity.lower(), 1) for s in signals[:10]) / min(10, len(signals))
