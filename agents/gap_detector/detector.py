...
class GapDetector(BaseAgentAsync):
    ...
    async def analyze(
        ...
        try:
            ...
            return AgentResult(
                agent_name=self.name,
                success=False,
                error=f"Failed to detect gaps: {str(e)}"
            )
    ...
    async def _detect_missing_signals(self, context: Context, result: GapAnalysisResult) -> None:
        ...
        for signal_type in missing_types:
            ...
            gap = DataGap(
                ...
                recommendations=self._generate_missing_signal_recommendations(signal_type, likely_sources),
    ...
    async def _detect_monitoring_gaps(self, context: Context, result: GapAnalysisResult) -> None:
        ...
        for monitoring_type in missing_monitoring:
            gap = DataGap(
                ...
                confidence=0.7,
                entity_id=context.entity.id,
                recommendations=self._generate_monitoring_recommendations(monitoring_type, entity_type),
                impact_score=0.6,
            )
            result.gaps.append(gap)
    ...
    def _generate_monitoring_recommendations(
        ...
        return monitoring_solutions.get(monitoring_type, [f"Implement {monitoring_type} monitoring"])
