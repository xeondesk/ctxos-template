...
class TestGapDetector:
    ...
    def test_detect_monitoring_gaps(self, gap_detector, sample_context):
        ...
        monitoring_gaps = [
            gap for gap in analysis["gaps"]
            if gap["gap_type"] == "monitoring_gap"
        ]
    ...
    def test_detect_correlation_gaps(self, gap_detector, sample_context):
        ...
        correlation_gaps = [
            gap for gap in analysis["gaps"]
            if gap["gap_type"] == "correlation_gap"
        ]
    ...
    def test_entity_type_requirements(self, gap_detector):
        ...
        detected_missing = {gap["expected_signals"][0] for gap in missing_signals if gap["expected_signals"]}
