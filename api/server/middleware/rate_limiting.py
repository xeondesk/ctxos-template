...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        for rule_key, rule in limit_rules.items():
            allowed, info = await self.limiter.is_allowed(
                f"{key}:{rule_key}", rule["limit"], rule["window"]
            )

            if not allowed:
                logger.warning(f"Rate limit exceeded for {key}: {rule}")
                raise RateLimitExceeded(rule["limit"], rule["window"], info["retry_after"])
    ...
    async def _check_global_limits(self, request: Request):
        ...
        if self.global_counters["requests"] > self.config.GLOBAL_LIMITS["requests_per_second"]:
            raise RateLimitExceeded(self.config.GLOBAL_LIMITS["requests_per_second"], 1, 1)

    async def _add_rate_limit_headers(
        self, response: Response, rules: Dict[str, Dict[str, int]], key: str
    ):
...
class CollectorRateLimiter:
    ...
    async def check_collector_limit(
        self, collector_type: str, tenant_id: Optional[int] = None
    ) -> Tuple[bool, Dict[str, int]]:
    ...
    async def record_collector_run(
        self, collector_type: str, tenant_id: Optional[int] = None, items_processed: int = 0
    ):
...
rate_limiter = RateLimitingMiddleware(
    app=None, config=RateLimitConfig()  # Will be set when middleware is added to FastAPI app
)
