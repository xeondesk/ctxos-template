...
TENANT_ROLE_PERMISSIONS = {
    "owner": [
        ...
        Permission.TENANT_MEMBERS_ADD,
        Permission.TENANT_MEMBERS_UPDATE,
        Permission.TENANT_MEMBERS_REMOVE,
    ...
    "viewer": [Permission.TENANT_READ, Permission.PROJECTS_READ, Permission.DATA_READ],
}
...
PROJECT_ROLE_PERMISSIONS = {
    ...
    "viewer": [Permission.PROJECT_READ, Permission.DATA_READ],
    "collaborator": [
        Permission.PROJECT_READ,
        Permission.DATA_READ,
        Permission.DATA_CREATE,
...
class EnhancedRBAC:
    ...
    def clear_cache(self, user_id: Optional[int] = None):
        ...
        if user_id:
            ...
            keys_to_remove = [
                k for k in self.permission_cache.keys() if k.startswith(f"{user_id}:")
            ]
    ...
    def get_accessible_resources(
        self, user_id: int, resource_type: ResourceType, action: Action, db_session=None
    ) -> List[int]:
        ...
        if db_session:
            if resource_type == ResourceType.TENANT:
                ...
                members = (
                    db_session.query(TenantMember).filter(TenantMember.user_id == user_id).all()
                )
                ...
                members = (
                    db_session.query(ProjectMember).filter(ProjectMember.user_id == user_id).all()
                )
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            member = (
                ...
                .filter(
                    TenantMember.tenant_id == tenant_id, TenantMember.user_id == current_user.id
                )
                .first()
            )
...
def require_project_role(role: Union[str, List[str]]):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not project_id:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="Project context required"
                )
...
async def get_current_user_permissions(
    ...
    return rbac.get_user_permissions(current_user.id, tenant_id, project_id, db)
...
async def check_user_permission(
    ...
    if not rbac.check_permission(current_user.id, permission, tenant_id, project_id, db):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail=f"Insufficient permissions: {permission}"
        )
...
def can_access_tenant(user_id: int, tenant_id: int, db_session) -> bool:
    ...
    return rbac.check_permission(user_id, Permission.TENANT_READ, tenant_id, None, db_session)


def can_access_project(user_id: int, project_id: int, db_session) -> bool:
    """Check if user can access project."""
    return rbac.check_permission(user_id, Permission.PROJECT_READ, None, project_id, db_session)
...
def can_manage_tenant(user_id: int, tenant_id: int, db_session) -> bool:
    ...
    return rbac.check_any_permission(
        user_id, [Permission.TENANT_UPDATE, Permission.TENANT_DELETE], tenant_id, None, db_session
    )
