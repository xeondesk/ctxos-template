...
class TenantIsolationMixin:
    ...
    @staticmethod
    def apply_tenant_filter(query, tenant_id: Optional[int] = None, project_id: Optional[int] = None):
        ...
        if tenant_id:
            ...
            query = query.filter(
                ...
                (getattr(query.column_descriptions[0]['type'], 'project_id').has(Project.tenant_id == tenant_id))
            )
    ...
    def validate_project_access(obj, project_id: int, user_id: int, db_session):
        ...
        try:
            await tenant_service._check_project_permission(
                project_id, user_id,
                ['owner', 'admin', 'member', 'viewer', 'collaborator']
            )
        except HTTPException:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
class TenantAwareSession:
    ...
    def query(self, *args, **kwargs):
        ...
        if self.tenant_id or self.project_id:
            query = TenantIsolationMixin.apply_tenant_filter(
                query, self.tenant_id, self.project_id
            )
