...
class SignalInput(BaseModel):
    ...
    properties: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Signal properties"
    )
...
class ScoreRequestBody(BaseModel):
    ...
    engines: Optional[List[str]] = Field(
        default=["risk", "exposure", "drift"], description="Engines to run (default: all)"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                "context": {
                    "entity": {"id": "host-001", "entity_type": "host", "name": "web-server-01"},
                    "signals": [
                        {
                            "id": "port-001",
                            "source": "nmap",
...
class BatchScoreRequest(BaseModel):
    ...
    contexts: List[ContextInput] = Field(
        ..., description="Entity contexts", min_items=1, max_items=100
    )
    engines: Optional[List[str]] = Field(
        default=["risk", "exposure", "drift"], description="Engines to run"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                "contexts": [
                    {"entity": {"id": "host-001", "entity_type": "host"}, "signals": []},
                    {"entity": {"id": "host-002", "entity_type": "host"}, "signals": []},
                ],
                "engines": ["risk"],
                "parallel": True,
            }
...
class AgentRunRequest(BaseModel):
    ...
    timeout_seconds: Optional[float] = Field(
        30.0, description="Execution timeout", ge=1.0, le=300.0
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "context": {"entity": {"id": "host-001", "entity_type": "host"}, "signals": []},
                "parallel": True,
                "timeout_seconds": 60,
            }
        }
...
class HistoricalQueryRequest(BaseModel):
    ...
    engines: Optional[List[str]] = Field(
        default=["risk", "exposure", "drift"], description="Engines to query"
    )
...
class BulkAnalysisRequest(BaseModel):
    ...
    agents: Optional[List[str]] = Field(
        default=["context_summarizer", "gap_detector"], description="Agents to run"
    )
    parallel: Optional[bool] = Field(True, description="Process in parallel")
    timeout_per_entity: Optional[float] = Field(
        30.0, description="Timeout per entity", ge=1.0, le=300.0
    )
