...
class EvidenceService:
    ...
    async def get_evidence(self, evidence_id: int) -> EvidenceResponse:
        ...
        if evidence.approval_workflow_id:
            workflow = (
                ...
                .filter(ApprovalWorkflow.id == evidence.approval_workflow_id)
                .first()
            )

        attachment_count = (
            self.db.query(EvidenceAttachment)
            .filter(EvidenceAttachment.evidence_id == evidence_id)
            .count()
        )

        approval_count = self.db.query(Approval).filter(Approval.evidence_id == evidence_id).count()
    ...
    async def update_evidence(
        self, evidence_id: int, evidence_data: EvidenceUpdate, user_id: int
    ) -> EvidenceResponse:
        """Update evidence."""
        evidence = self.db.query(Evidence).filter(Evidence.id == evidence_id).first()
        if not evidence:
    ...
    async def search_evidence(
        self, search_params: EvidenceSearch, user_id: int
    ...
    async def create_approval_workflow(
        self, workflow_data: ApprovalWorkflowCreate, creator_id: int
    ) -> ApprovalWorkflowResponse:
    ...
    async def get_approval_workflow(self, workflow_id: int) -> ApprovalWorkflowResponse:
        ...
        workflow = (
            self.db.query(ApprovalWorkflow).filter(ApprovalWorkflow.id == workflow_id).first()
        )
        if not workflow:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Approval workflow not found"
            )

        # Get related data
        creator = self.db.query(User).filter(User.id == workflow.created_by).first()
        step_count = (
            self.db.query(ApprovalStep).filter(ApprovalStep.workflow_id == workflow_id).count()
        )

        evidence_count = (
            self.db.query(Evidence).filter(Evidence.approval_workflow_id == workflow_id).count()
        )
    ...
    async def create_approval_step(self, step_data: ApprovalStepCreate, creator_id: int) -> bool:
        ...
        workflow = (
            self.db.query(ApprovalWorkflow)
            .filter(ApprovalWorkflow.id == step_data.workflow_id)
            .first()
        )
        if not workflow:
            raise HTTPException(
    ...
    async def create_approval(
        self, evidence_id: int, workflow_id: int, step_id: int, approver_id: int
    ) -> ApprovalResponse:
        ...
        if not evidence.requires_approval:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Evidence does not require approval"
            )
        ...
        if existing:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT, detail="Approval already exists"
            )
    ...
    async def update_approval(
        self, approval_id: int, approval_data: ApprovalUpdate, user_id: int
    ) -> ApprovalResponse:
    ...
    async def upload_attachment(
        self, evidence_id: int, file: UploadFile, user_id: int
    ...
    async def get_attachment(self, attachment_id: int) -> EvidenceAttachmentResponse:
        ...
        attachment = (
            self.db.query(EvidenceAttachment).filter(EvidenceAttachment.id == attachment_id).first()
        )
        if not attachment:
            raise HTTPException(
    ...
    async def get_audit_trail(self, evidence_id: int) -> AuditTrail:
            ...
            event_history.append(
                {
                    ...
                    "details": {"notes": evidence.approval_notes},
                }
            )
        ...
        approvals = self.db.query(Approval).filter(Approval.evidence_id == evidence_id).all()
    ...
    async def _check_approval_permission(self, evidence_id: int, step_id: int, user_id: int):
        ...
        if not step:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Approval step not found"
            )
    ...
    async def _get_approval_workflow(
        self, tenant_id: int, evidence_type: EvidenceType
    ) -> Optional[int]:
