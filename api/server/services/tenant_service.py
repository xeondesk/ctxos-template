...
from ..models.tenant import (
    ...
    TenantCreate, TenantUpdate, ProjectCreate, ProjectUpdate,
    TenantMemberCreate, TenantMemberUpdate, ProjectMemberCreate, ProjectMemberUpdate,
    TenantResponse, ProjectResponse, UserContext, UserTenantAccess, UserProjectAccess,
    TenantStatus, ProjectStatus, TenantRole, ProjectRole
)
...
class TenantService:
    ...
    async def get_tenant(self, tenant_id: int) -> TenantResponse:
        ...
        # Get counts
        member_count = self.db.query(TenantMember).filter(
    ...
    async def update_tenant(self, tenant_id: int, tenant_data: TenantUpdate, user_id: int) -> TenantResponse:
        """Update tenant."""
        # Check permissions
        await self._check_tenant_permission(tenant_id, user_id, [TenantRole.OWNER, TenantRole.ADMIN])
    ...
    async def create_project(self, project_data: ProjectCreate, creator_id: int) -> ProjectResponse:
        ...
        await self._check_tenant_permission(
            project_data.tenant_id, creator_id, 
        ...
        if project_count >= tenant.project_limit:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Tenant project limit reached"
            )
    ...
    async def update_project(self, project_id: int, project_data: ProjectUpdate, user_id: int) -> ProjectResponse:
        """Update project."""
        await self._check_project_permission(project_id, user_id, [ProjectRole.OWNER, ProjectRole.ADMIN])
    ...
    async def list_tenant_projects(self, tenant_id: int, user_id: int) -> List[ProjectResponse]:
        ...
        await self._check_tenant_permission(tenant_id, user_id, [TenantRole.OWNER, TenantRole.ADMIN, TenantRole.MEMBER])
    ...
    # Member Management
    async def add_tenant_member(self, tenant_id: int, member_data: TenantMemberCreate, user_id: int) -> bool:
        ...
        existing = self.db.query(TenantMember).filter(
            and_(
                TenantMember.tenant_id == tenant_id,
    ...
    async def update_tenant_member(self, tenant_id: int, user_id: int, member_data: TenantMemberUpdate, current_user_id: int) -> bool:
        """Update tenant member role."""
        await self._check_tenant_permission(tenant_id, current_user_id, [TenantRole.OWNER, TenantRole.ADMIN])
    ...
    async def remove_tenant_member(self, tenant_id: int, user_id: int, current_user_id: int) -> bool:
    ...
    async def add_project_member(self, project_id: int, member_data: ProjectMemberCreate, user_id: int) -> bool:
    ...
    async def update_project_member(self, project_id: int, user_id: int, member_data: ProjectMemberUpdate, current_user_id: int) -> bool:
        """Update project member role."""
        await self._check_project_permission(project_id, current_user_id, [ProjectRole.OWNER, ProjectRole.ADMIN])
    ...
    async def remove_project_member(self, project_id: int, user_id: int, current_user_id: int) -> bool:
    ...
    async def get_user_context(self, user_id: int) -> UserContext:
        ...
        for member in tenant_members:
            ...
            if tenant:
                ...
                tenants.append(UserTenantAccess(
                    tenant_id=tenant.id,
        ...
        project_members = self.db.query(ProjectMember).filter(ProjectMember.user_id == user_id).all()
        projects = []
        ...
        for member in project_members:
            ...
            if project:
                ...
                projects.append(UserProjectAccess(
                    project_id=project.id,
    ...
    # Helper Methods
    async def _check_tenant_permission(self, tenant_id: int, user_id: int, required_roles: List[TenantRole]) -> None:
    ...
    async def _check_project_permission(self, project_id: int, user_id: int, required_roles: List[ProjectRole]) -> None:
    ...
    async def _get_tenant_permissions(self, role: TenantRole) -> List[str]:
        ...
        permissions = {
            TenantRole.OWNER: [
                ...
                "tenant:members:add", "tenant:members:update", "tenant:members:remove",
