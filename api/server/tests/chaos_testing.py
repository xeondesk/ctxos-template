...
class ChaosInjector:
    ...
    def inject_collector_failure(self, collector_name: str, failure_type: str = "timeout") -> Dict[str, Any]:
        ...
        if failure_type == "timeout":
            mock_method = unittest.mock.MagicMock(side_effect=asyncio.TimeoutError("Collector timeout"))
    ...
    def inject_data_corruption(self, data_source: str, corruption_type: str = "random") -> Dict[str, Any]:
...
class ChaosTester:
    ...
    def run_chaos_test(self, config: ChaosConfig, test_function: Optional[Callable] = None) -> ChaosResult:
        ...
        logger.info(f"Chaos test completed: {config.scenario} - {'SUCCESS' if result.success else 'FAILED'}")
    ...
    def _inject_chaos(self, config: ChaosConfig) -> Dict[str, Any]:
        ...
        if config.scenario == ChaosScenario.COLLECTOR_FAILURE:
            target = config.target_components[0] if config.target_components else "default_collector"
            ...
            target = config.target_components[0] if config.target_components else "default_data_source"
            ...
            memory_mb = 512 if config.severity == Severity.HIGH else 256 if config.severity == Severity.MEDIUM else 128
            return self.injector.inject_memory_pressure(memory_mb)
        
        elif config.scenario == ChaosScenario.DISK_FULL:
            size_mb = 1024 if config.severity == Severity.HIGH else 512 if config.severity == Severity.MEDIUM else 256
    ...
    def _create_test_database(self, db_path: str):
        ...
        # Insert test data
        cursor.execute("INSERT INTO test_entities (name, data, created_at) VALUES (?, ?, ?)",
                       ("test_entity", b"test_data", datetime.now()))
        
        cursor.execute("INSERT INTO test_signals (entity_id, signal_type, value, timestamp) VALUES (?, ?, ?, ?)",
    ...
    def generate_report(self, results: Optional[List[ChaosResult]] = None) -> Dict[str, Any]:
        ...
        avg_impact_score = sum(imp.get("overall_impact_score", 0) for imp in all_impacts) / len(all_impacts) if all_impacts else 0
