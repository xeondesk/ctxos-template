    ...
    return {
        ...
        "signals": [
            ...
            },
            {
                ...
                "properties": {"port": 443, "service": "https", "state": "open"},
            },
            {
                "id": "config-001",
                "source": "config_audit",
                ...
                "properties": {"cert_expiry_days": 15, "cert_issuer": "Let's Encrypt"},
            },
        ],
    }
    ...
    def test_config_and_agent_integration(self, client, admin_token, sample_context):
        ...
        rule_response = client.post(
            ...
            json={
                ...
                "condition": {"entity_type": "host", "min_critical_vulns": 1},
                "action": {
                    "risk_multiplier": 2.0,
    ...
    def test_error_handling_and_recovery(self, client, admin_token):
        ...
        invalid_agent_response = client.post(
            ...
            json={
                ...
                "context": {"entity": {"id": "test", "entity_type": "host"}, "signals": []},
            },
        )
        ...
        invalid_config_response = client.post(
            "/api/v1/config/update",
            headers={"Authorization": f"Bearer {admin_token}"},
            json={"config_key": "nonexistent.key", "value": "test"},
        )
...
class TestCrossServiceIntegration:
    ...
    def test_auth_to_config_to_agents_flow(self, client):
        ...
        config_response = client.get("/api/v1/config", headers={"Authorization": f"Bearer {token}"})

        assert config_response.status_code == 200

        # Step 3: Use same token to run agents
        agent_response = client.post(
            "/api/v1/agents/list", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_configuration_affects_all_services(self, client, admin_token):
        ...
        config_response = client.post(
            "/api/v1/config/update",
            headers={"Authorization": f"Bearer {admin_token}"},
            json={"config_key": "logging.level", "value": "DEBUG"},
        )
        ...
        get_config_response = client.get(
            "/api/v1/config/logging.level", headers={"Authorization": f"Bearer {admin_token}"}
        )
...
class TestSecurityIntegration:
    ...
    def test_permission_enforcement_across_services(self, client, analyst_token, sample_context):
        ...
        for endpoint, method in read_endpoints:
            if method == "GET":
                response = client.get(
                    endpoint, headers={"Authorization": f"Bearer {analyst_token}"}
                )
            else:
        ...
        for endpoint, method, data in write_endpoints:
            response = client.post(
                endpoint, headers={"Authorization": f"Bearer {analyst_token}"}, json=data
            )
