...
class TestTenantCreation:
    ...
    def test_create_tenant_success(self, test_user):
        ...
        response = client.post(
            "/tenants/", json=tenant_data, headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_create_tenant_auto_slug(self, test_user):
        ...
        tenant_data = {"name": "Auto Slug Tenant", "description": "Test auto slug generation"}
    ...
    def test_list_user_tenants(self, test_user):
        ...
        tenant_data = {"name": "List Test Tenant", "description": "For listing test"}
        client.post("/tenants/", json=tenant_data, headers={"Authorization": f"Bearer {token}"})
    ...
    def test_get_tenant_success(self, test_user):
        ...
        # Create a tenant
        tenant_data = {"name": "Get Test Tenant", "description": "For get test"}
        ...
        # Get tenant
        response = client.get(f"/tenants/{tenant_id}", headers={"Authorization": f"Bearer {token}"})
    ...
    def test_get_tenant_unauthorized(self, test_user):
        ...
        response = client.get("/tenants/999", headers={"Authorization": f"Bearer {token}"})
    ...
    def test_update_tenant_success(self, test_user):
        ...
        # Create a tenant
        tenant_data = {"name": "Update Test Tenant", "description": "For update test"}
        create_response = client.post(
            "/tenants/", json=tenant_data, headers={"Authorization": f"Bearer {token}"}
        )
        tenant_id = create_response.json()["id"]
        ...
        # Update tenant
        update_data = {"name": "Updated Tenant Name", "description": "Updated description"}
        response = client.put(
            f"/tenants/{tenant_id}", json=update_data, headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_delete_tenant_success(self, test_user):
        ...
        # Create a tenant
        tenant_data = {"name": "Delete Test Tenant", "description": "For delete test"}
        create_response = client.post(
            "/tenants/", json=tenant_data, headers={"Authorization": f"Bearer {token}"}
        )
        tenant_id = create_response.json()["id"]
...
class TestProjectManagement:
    ...
    def test_create_project_success(self, test_user):
        ...
        # Create a tenant first
        tenant_data = {"name": "Project Test Tenant", "description": "For project tests"}
    ...
    def test_list_tenant_projects(self, test_user):
        ...
        # Create a tenant
        tenant_data = {"name": "Project List Tenant", "description": "For project list test"}
        tenant_response = client.post(
            "/tenants/", json=tenant_data, headers={"Authorization": f"Bearer {token}"}
        )
        tenant_id = tenant_response.json()["id"]
        ...
        # Create projects
        for i in range(3):
            project_data = {"name": f"Project {i}", "description": f"Test project {i}"}
            client.post(
                f"/tenants/{tenant_id}/projects",
                json=project_data,
                headers={"Authorization": f"Bearer {token}"},
        ...
        # List projects
        response = client.get(
            f"/tenants/{tenant_id}/projects", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_delete_project_success(self, test_user):
        ...
        # Delete project
        response = client.delete(
            f"/tenants/projects/{project_id}", headers={"Authorization": f"Bearer {token}"}
        )
...
class TestMemberManagement:
    ...
    def test_add_tenant_member(self, test_user):
        ...
        # Add member
        member_data = {"user_id": other_user.id, "role": TenantRole.MEMBER}
        response = client.post(
    ...
    def test_add_project_member(self, test_user):
        ...
        # Add member
        member_data = {"user_id": other_user.id, "role": ProjectRole.MEMBER}
        response = client.post(
...
class TestUserContext:
    ...
    def test_get_user_context(self, test_user):
        ...
        # Get user context
        response = client.get("/tenants/context/user", headers={"Authorization": f"Bearer {token}"})
...
class TestTenantIsolation:
    ...
    def test_tenant_isolation(self, test_user):
        ...
        tenant1_response = client.post(
            "/tenants/", json={"name": "User1 Tenant"}, headers={"Authorization": f"Bearer {token}"}
        )
        ...
        user2_response = client.post(
            "/auth/login", data={"username": "user2", "password": "testpassword"}
        )
        ...
        response = client.get(
            f"/tenants/{tenant2_id}", headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == 404

        # User2 should not be able to access User1's tenant
        response = client.get(
            f"/tenants/{tenant1_id}", headers={"Authorization": f"Bearer {user2_token}"}
        )
        assert response.status_code == 404
