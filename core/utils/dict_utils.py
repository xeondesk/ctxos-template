...
def sanitize_dict(data: Dict[str, Any], keys_to_remove: List[str] = None) -> Dict[str, Any]:
    ...
    if keys_to_remove is None:
        keys_to_remove = [
            ...
            "credential", "auth", "access_token", "refresh_token",
            "_internal", "_debug",
        ]
    ...
    for key, value in data.items():
        ...
        if isinstance(value, dict):
            ...
            result[key] = [sanitize_dict(item, keys_to_remove) if isinstance(item, dict) else item for item in value]
        else:
            result[key] = value
...
def convert_timestamps(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        if isinstance(value, str):
            ...
            result[key] = [convert_timestamps(item) if isinstance(item, dict) else item for item in value]
        else:
            result[key] = value
