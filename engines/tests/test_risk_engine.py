...
from core.models import Entity, EntityType, Signal, SignalType, SignalSeverity, Context, EntitySeverity
...
class TestRiskEngine:
    ...
    def test_severity_thresholds(self, engine):
        ...
        for severity, (low, high) in thresholds.items():
            ...
            detected_severity = engine._determine_severity(test_score) if hasattr(engine, '_determine_severity') else \
                               ("critical" if test_score >= 80 else
                                "high" if test_score >= 60 else
                                "medium" if test_score >= 40 else
                                "low" if test_score >= 20 else "info")
