...
class TestNormalizationWorkflow:
    ...
    def test_end_to_end_entity_normalization(self):
        ...
        # Register rules
        rule_engine.register_rule(DefaultValueRule({"status": "active", "confidence": 0.0}))
        ...
        # 1. Map fields
        mapped_entities = [mapper.map_entity("dns_source", entity) for entity in raw_entities]

        # 2. Normalize
        normalized_entities = [engine.normalize_entity(entity) for entity in mapped_entities]

        # 3. Apply rules
        rule_applied_entities = [rule_engine.apply_rules(entity) for entity in normalized_entities]
    ...
    def test_signal_normalization_pipeline(self):
        ...
        # Process pipeline
        mapped_signals = [mapper.map_signal("firewall", signal) for signal in raw_signals]
        normalized_signals = [engine.normalize_signal(signal) for signal in mapped_signals]
        processed_signals = [rule_engine.apply_rules(signal) for signal in normalized_signals]
    ...
    def test_mixed_source_normalization(self):
        ...
        mapper.register_mapping("source_a", {"domain": "name", "found_date": "discovered"})
        mapper.register_mapping("source_b", {"hostname": "name", "first_seen": "discovered"})

        entities_a = [{"domain": "example.com", "found_date": "2024-01-01"}]
        entities_b = [{"hostname": "example.com", "first_seen": "2024-01-01"}]
